<template>
  <div class="chat">
    <el-scrollbar class="chat-messages" ref="scrollbarRef">
      <RecommandPage
        @tryTheExample="selectExample"
        v-show="isShowRecommendPage || messages.length === 0"
      />
      <div
        v-for="(msg, i) in messages"
        :key="msg.id"
        v-show="!isShowRecommendPage"
        style="display: flex"
        :style="
          msg.role === 'user'
        ...
        "
      >
        <div class="avatar" v-show="msg.role === 'assistant'">
          <img :src="gptAvatar" />
        </div>
        <el-card
          shadow="hover"
          :style="{
            ...
            color: msg.role === 'user' ? 'var(--el-text-color-primary)' : '',
          }"
          :class="[
            'message',
        ...
        >
          <div class="content-operation">
            ...
            <el-popconfirm
              ...
              icon-color="#626AEF"
              title="确定删除当前记录消息吗?"
              @cancel="deleteMsgConfirm(msg.id)"
            >
              <template #reference>
                <el-icon class="oper-btn delete-btn" title="删除消息"
                  ><Delete />
                </el-icon>
              </template>
            </el-popconfirm>
        ...
        <div class="avatar"  v-show="msg.role === 'user'">
          <div class="emoji">
            <figure class="face">
              <span class="eyes">
                <span class="eye"></span>
      ...
      <LoadingTip v-show="isLoading" />
    </el-scrollbar>
    <div class="chat-input">
      <el-icon
        @click="openPromptRepo"
      ...
      <el-input
        ...
        :disabled="isLoading"
        type="text"
        placeholder="请输入要查询的内容， 按下 Ctrl + Enter 发送"
      >
        <template #append>
          <el-button
            :icon="MagicStick"
    ...
    <el-drawer
      v-model="isShowPromptRepo"
      title="Prompt 仓库"
      :with-header="false"
      size="30%"
    >
      <!-- <el-card class="box-card" shadow="never">
        <div class="prompt-repo-header">
          <p style="width: 100%; white-space: normal">
            prompt 是指输入给 gpt 的文本，本仓库收集了一些常用的 prompt，你可以点击下面的按钮将其添加到输入框中。
          </p>
        </div>
      </el-card> -->
...
import LoadingTip from "./loading-tip.vue";
import { nextTick } from "process";
import RecommandPage from "./recommand-page.vue";
...
// 显示/隐藏 prompt drawer 的控制变量
const isShowPromptRepo = ref(false);

// 打开 prompt 抽屉
const openPromptRepo = () => {
  isShowPromptRepo.value = true;
};
// 选中 prompt
const selectPrompt = (prompt: string) => {
  inputText.value = prompt;
  isShowPromptRepo.value = false;
};

// prompt list
const prompList = Object.freeze([
  {
    act: "雅思写作考官",
    prompt:
...
// 是否显示推荐页面
const isShowRecommendPage = ref(false);
...
const scrollToBottom = () => {
  nextTick(() => {
    const el = document.querySelector(".el-scrollbar__view") as HTMLElement;
...
const updateChatHistory = (message: Message) => {
  chat.addRecord(message);
};
// 获取最后一条消息
const getLastMessage = (parsedObjects) => {
  const { id, text } = parsedObjects[parsedObjects.length - 1];
  return { id, text, role: "assistant" };
};
...
// 发送消息
const sendMessage = () => {
  const iptText = inputText.value.trim();
  if (!iptText) {
    // 清除已有提示，避免重复提示
    ElMessage.closeAll();
    return ElMessage({
  ...
  if (iptText) {
    const id = nanoid();
    updateChatHistory({ id, text: iptText, role: "user" });
    scrollToBottom();
  }
  ...
  if (messages.value.length < 14) {
    newArr = messages.value.map(({ text, role }) => ({ content: text, role }));
  } else {
    newArr = messages.value
      .slice(-14)
      .map(({ text, role }) => ({ content: text, role }));
  }
  ...
  let str = "";
  const id = nanoid();
  fetch("https://openai.com/v1/chat/completions", requestOptions)
    .then((stream: any) => {
      ...
      reader.read().then(function processText({ done, value }) {
        ...
        const dataBlocks = chunk.split("data:").slice(1);
        for (const dataBlock of dataBlocks) {
          ...
          try {
            ...
            // 使用正则表达式查找第一个test的值
            const match = dataBlock.match(/"content":\s*"([^"]+)"/);
          ...
          str += data?.choices[0]?.delta?.content || previousStr || "";
          if (messages.value[messages.value.length - 1].id !== id) {
    ...
    .catch((error) => {
      ...
      chat.setLoading(false);
      // 将最后一条消息内容放置到输入框中
      inputText.value = messages.value[messages.value.length - 1].text;
      // 删除messags中的最后一条
      messages.value.pop();
...
<style>
/* :class="isLoading && i === messages.length - 1 && messages[messages.length - 1].role === 'assistant' ?  'v-md-preview' : ''"  */
.v-md-preview {
  position: relative;
}
.v-md-preview::after {
...
.emoji:hover .mouth.tounge:after {
  animation-name: tounge-wave;
  animation-iteration-count: infinite;
  animation-duration: 2s;
}
</style>